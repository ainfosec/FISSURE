...
unsigned int constellation_bpsk_impl::decision_maker(const gr_complex* sample)
{
...
unsigned int constellation_qpsk_impl::decision_maker(const gr_complex* sample)
{
...
unsigned int constellation_16qam_impl::decision_maker(const gr_complex* sample)
{
    unsigned int ret = 0;
    const float level = sqrt(float(0.1));
...
constellation_64qam::sptr constellation_64qam::make()
{
