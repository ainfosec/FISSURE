...
uc_connection::uc_connection(rime_stack* block,
                             uint16_t channel,
                             pmt::pmt_t inport,
                             pmt::pmt_t outport,
...
uc_connection::make_msgbuf(uint16_t channel, const uint8_t src[2], const uint8_t dest[2])
{
...
bool uc_connection::rime_add_from_string(std::string& to_parse, uint8_t addr[2])
{
...
void uc_connection::pack(pmt::pmt_t msg)
{
    ...
    std::array<uint8_t, 256> buf =
        uc_connection::make_msgbuf(d_channel, d_rime_add_mine, dest);
...
void uc_connection::unpack(pmt::pmt_t msg)
{
