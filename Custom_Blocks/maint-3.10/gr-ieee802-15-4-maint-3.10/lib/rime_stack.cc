...
public:
    rime_stack_impl(std::vector<uint16_t> bc_channels,
                ...
                gr::io_signature::make(0, 0, 0))
    {
        ...
        set_msg_handler(
            mac_in, boost::bind(&rime_stack_impl::unpack, this, boost::placeholders::_1));
    ...
    void add_connections(int num_conns,
                         std::vector<uint16_t>& channels,
                         const char* inport_base,
                         const char* outport_base,
                         conn_type type)
    {
        ...
        for (int i = 0; i < num_conns; i++) {
            ...
            switch (type) {
            case bc: {
                bc_connection* to_add =
                    new bc_connection(this, channels[i], inport, outport, d_rime_add);
                set_msg_handler(
                    inport,
            ...
            case uc: {
                uc_connection* to_add =
                    new uc_connection(this, channels[i], inport, outport, d_rime_add);
                set_msg_handler(
                    inport,
            ...
            case ruc: {
                ruc_connection* to_add =
                    new ruc_connection(this, channels[i], inport, outport, d_rime_add);
                set_msg_handler(
                    inport,
...
rime_stack::sptr rime_stack::make(std::vector<uint16_t> bc_channels,
                                  std::vector<uint16_t> uc_channels,
                                  std::vector<uint16_t> ruc_channels,
                                  std::vector<uint8_t> rime_add)
{
    return gnuradio::get_initial_sptr(
        new rime_stack_impl(bc_channels, uc_channels, ruc_channels, rime_add));
}
