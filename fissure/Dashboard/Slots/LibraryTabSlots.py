...
async def _slotLibraryAddAddToLibrary_Clicked(dashboard: QtCore.QObject):
    ...
    if dashboard.ui.stackedWidget2_library_pd.currentIndex() == 0:
        ...
        if (len(protocol_name) == 0) or (protocol_name in protocols):
            dashboard.errorMessage("Enter valid protocol name.")
            return
    else:
    ...
    if dashboard.ui.stackedWidget2_library_pd.currentIndex() == 1:
        ...
        if (len(get_modulation) == 0) or (get_modulation in modulation_types):
            dashboard.errorMessage("Enter valid modulation type.")
            return
    ...
    # Packet Type
        ...
        if (len(new_packet_name) == 0) or (new_packet_name in packet_types):
            dashboard.errorMessage("Enter valid packet name.")
            return

        else:
            # Check for Content
            if dashboard.ui.tableWidget_library_pd_packet.rowCount() == 0:
                dashboard.errorMessage("The number of fields cannot be zero.")
                return
            else:
                # Get Packet Data
                for row in range(0,dashboard.ui.tableWidget_library_pd_packet.rowCount()):
                    ...
                    if set(check_default_values).issubset({'0','1',' '}) and bool(check_default_values):
                        ...
                        dashboard.errorMessage("Default values must be binary: 1010 0010 1111...")
                        return
    ...
    # Signal of Interest
        ...
        if len(str(dashboard.ui.textEdit_library_pd_soi_subtype.toPlainText()).replace(" ","")) == 0:
            dashboard.errorMessage("Requires Subtype/Label")
            return
    ...
    # Demodulation Flow Graph
        ...
        if get_demodulation_fg == "":
            dashboard.errorMessage("Enter valid demodulation flow graph filepath.")
            return
        if get_demodulation_type == "":
            dashboard.errorMessage("Add modulation type for demodulation flow graph.")
            return
        ...
        try:
            ...
            if demod_fg_exists:
                dashboard.errorMessage("Duplicate demodulation flow graph name")
                return
            ...
            dashboard.errorMessage("New demodulation flow graph requires a valid .py and .grc file with the same name.")
            return
    ...
    # Attack
        ...
        if len(get_filepath) == 0:
            dashboard.errorMessage('Select attack file.')
            return

        # Invalid Attack Template Name
        if len(get_attack_name) == 0:
            dashboard.errorMessage('Enter new attack template name.')
            return

        # Invalid Attack Name
        if len(get_new_filename) == 0:
            dashboard.errorMessage('Enter new attack name.')
            return
        ...
        if get_filepath.rsplit(".",1)[-1] == "py":
            ...
            dashboard.errorMessage('Attack needs to end with ".py" or ".msa"')
            return
        ...
        if attack_type == "Single-Stage":
            ...
            if len(get_modulation[0]) == 0:
                dashboard.errorMessage('Requires modulation type.')
                return
        ...
        if os.path.isfile(os.path.join(fissure.utils.get_fg_library_dir(dashboard.backend.os_info), "Single-Stage Flow Graphs", get_new_filename)):
            ...
            dashboard.errorMessage('File already exists in "Single-Stage Flow Graphs" folder.')
            return
        else:
            ...
            if protocol_attack_exists == False:
                ...
                #for n in current_tree_attacks:
                        ...
                        #dashboard.errorMessage("Warning: Attack already exists in Tree Widget")
                        #no_errors = False
                ...
                #dashboard.errorMessage("Imported Successfully!")  # Needs to happen on component return message
                dashboard.ui.label_library_attacks_filepath.setText("")
            ...
            # Attack Already Exists for the Protocol
                ...
                dashboard.errorMessage("Attack name already exists for this protocol/modulation/hardware combination.")
                return
        return
...
async def _slotLibraryRemoveProtocolClicked(dashboard: QtCore.QObject):
    ...
    ret = await dashboard.ask_confirmation("Remove protocol and all its data from library?")
