...
class power_squelch_lowpass(gr.top_block):
    ...
    def __init__(self, beta='6.76', cutoff_freq='100e3', filepath="/home/user/Conditioner/Data/tpms/four_tires2.iq", sample_rate='1e6', squelch='-70', threshold='.002', transition_width='10e3'):
        ...
        self.blocks_tagged_file_sink_0_0 = blocks.tagged_file_sink(gr.sizeof_short*1, int(float(sample_rate)))
        ...
        self.analog_pwr_squelch_xx_0 = analog.pwr_squelch_cc(float(squelch), .001, 100, True)
        ...
        self.connect((self.blocks_complex_to_interleaved_short_0, 0), (self.blocks_tagged_file_sink_0_0, 0))
    ...
    def set_beta(self, beta):
        self.beta = beta
        self.low_pass_filter_0.set_taps(firdes.low_pass(1, float(self.sample_rate), float(self.cutoff_freq), float(self.transition_width), window.WIN_HAMMING, float(self.beta)))
...
def argument_parser():
    ...
    parser.add_argument(
        "--squelch", dest="squelch", type=str, default='-70',
...
def main(top_block_cls=power_squelch_lowpass, options=None):
    ...
    tb = top_block_cls(beta=options.beta, cutoff_freq=options.cutoff_freq, filepath=options.filepath, sample_rate=options.sample_rate, squelch=options.squelch, threshold=options.threshold, transition_width=options.transition_width)
