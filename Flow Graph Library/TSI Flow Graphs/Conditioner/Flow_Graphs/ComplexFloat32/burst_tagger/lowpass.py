...
class lowpass(gr.top_block):
    ...
    def __init__(self, transition_width="10e3", cutoff_freq="100000", filepath="", beta="6.76", threshold="0.002", sample_rate="1e6"):
...
def main(top_block_cls=lowpass, options=None):
    ...
    tb = top_block_cls(transition_width=options.transition_width, cutoff_freq=options.cutoff_freq, filepath=options.filepath, beta=options.beta, threshold=options.threshold, sample_rate=options.sample_rate)
    tb.start()
    tb.wait()
