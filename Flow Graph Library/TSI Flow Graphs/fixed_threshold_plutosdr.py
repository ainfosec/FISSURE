...
# Title: Fixed Threshold Plutosdr
...
class fixed_threshold_plutosdr(gr.top_block, Qt.QWidget):

    def __init__(self, antenna_default='', channel_default='', gain_default='64', ip_address='192.168.2.1', rx_freq_default='2412', sample_rate_default='20e6', serial='', threshold_default='0'):
        gr.top_block.__init__(self, "Fixed Threshold Plutosdr")
        Qt.QWidget.__init__(self)
        self.setWindowTitle("Fixed Threshold Plutosdr")
        ...
        self.settings = Qt.QSettings("GNU Radio", "fixed_threshold_plutosdr")
        ...
        for c in range(0, 1):
            self.top_grid_layout.setColumnStretch(c, 1)
        self._rx_gain_range = Range(0, 71, 1, float(gain_default), 200)
        ...
        for c in range(0, 4):
            self.top_grid_layout.setColumnStretch(c, 1)
        self.pluto_source_0 = iio.pluto_source("ip:" + str(ip_address), int(rx_freq*1e6), int(samp_rate), 20000000, 0x8000, True, True, True, "manual", rx_gain, '', True)
        ...
        self.connect((self.pluto_source_0, 0), (self.blocks_stream_to_vector_decimator_0, 0))
    ...
    def set_samp_rate(self, samp_rate):
        ...
        self._samp_rate_callback(self.samp_rate)
        self.pluto_source_0.set_params(int(self.rx_freq*1e6), int(self.samp_rate), 20000000, True, True, True, "manual", self.rx_gain, '', True)
...
def argument_parser():
    ...
    parser.add_option(
        "", "--antenna-default", dest="antenna_default", type="string", default='',
        help="Set antenna_default [default=%default]")
    parser.add_option(
        "", "--channel-default", dest="channel_default", type="string", default='',
        help="Set channel_default [default=%default]")
    parser.add_option(
        "", "--gain-default", dest="gain_default", type="string", default='64',
        help="Set 64 [default=%default]")
    parser.add_option(
...
def main(top_block_cls=fixed_threshold_plutosdr, options=None):
    if options is None:
