...
def getArguments():
    ...
    arg_names = ['json_filepath','artist_or_title','frequency_MHz','gain','timeout_seconds','wav1_filepath','run_with_sudo','notes']
    arg_values = [json_filepath, artist_or_title, frequency_MHz, gain, timeout_seconds, wav1_filepath, run_with_sudo, notes]
    ...
    if len(get_filepath) > 0:
        with open(get_filepath) as json_file:  
            ...
            for string in blns:                                                            # will loop through strings
                ...
                arguments = ['--' + get_artist_or_title + '=' + string, '--frequency=' + str(get_frequency), '--gain=' + str(get_gain), '--wav1-filepath=' + get_wav1_filepath]
                command = ['python2', script_path] + arguments                                                                
