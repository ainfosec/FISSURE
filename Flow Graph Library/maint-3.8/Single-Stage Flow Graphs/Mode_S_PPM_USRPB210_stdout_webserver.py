...
# Title: Mode S Ppm Usrpb210 Stdout Webserver
...
class Mode_S_PPM_USRPB210_stdout_webserver(gr.top_block):

    def __init__(self, antenna="TX/RX", channel="A:A", freq='1090e6', gain='70', ip_address="127.0.0.1", port='5001', samp_rate='2e6', serial="False", threshold='0.01'):
        gr.top_block.__init__(self, "Mode S Ppm Usrpb210 Stdout Webserver")
        ...
        # Variables
        ##################################################
        self.notes = notes = "Forwards formatted decoded ADSB data (gr-adsb) originating from an SDR (2 MS/s) to a webserver (tcp://127.0.0.1:5001) over ZMQ."
        ...
        self.uhd_usrp_source_0_0.set_subdev_spec(str(channel), 0)
        self.uhd_usrp_source_0_0.set_center_freq(float(freq), 0)
        self.uhd_usrp_source_0_0.set_gain(float(gain), 0)
        self.uhd_usrp_source_0_0.set_antenna(str(antenna), 0)
        self.uhd_usrp_source_0_0.set_samp_rate(float(samp_rate))
    ...
    def set_antenna(self, antenna):
        self.antenna = antenna
        self.uhd_usrp_source_0_0.set_antenna(str(self.antenna), 0)
    ...
    def set_freq(self, freq):
        self.freq = freq
        self.uhd_usrp_source_0_0.set_center_freq(float(self.freq), 0)
    ...
    def set_samp_rate(self, samp_rate):
        self.samp_rate = samp_rate
        self.uhd_usrp_source_0_0.set_samp_rate(float(self.samp_rate))
...
def main(top_block_cls=Mode_S_PPM_USRPB210_stdout_webserver, options=None):
    if options is None:
        options = argument_parser().parse_args()
    tb = top_block_cls(antenna=options.antenna, channel=options.channel, freq=options.freq, gain=options.gain, ip_address=options.ip_address, port=options.port, samp_rate=options.samp_rate, serial=options.serial, threshold=options.threshold)
