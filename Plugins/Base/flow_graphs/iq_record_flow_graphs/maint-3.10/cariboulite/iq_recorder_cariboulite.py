...
class iq_recorder_cariboulite(gr.top_block):
    ...
    def __init__(self, file_length="100000", filepath="", rx_antenna="", rx_frequency="2412", rx_gain="25", sample_rate="1", serial="0"):
        ...
        self.soapy_custom_source_0.set_sample_rate(0, float(sample_rate)*1e6)
...
def main(top_block_cls=iq_recorder_cariboulite, options=None):
    ...
    tb = top_block_cls(file_length=options.file_length, filepath=options.filepath, rx_antenna=options.rx_antenna, rx_frequency=options.rx_frequency, rx_gain=options.rx_gain, sample_rate=options.sample_rate, serial=options.serial)
