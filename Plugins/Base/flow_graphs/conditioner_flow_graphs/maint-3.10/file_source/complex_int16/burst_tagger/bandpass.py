...
class bandpass(gr.top_block):
    ...
    def set_bandpass_freq(self, bandpass_freq):
        self.bandpass_freq = bandpass_freq
        self.band_pass_filter_0.set_taps(firdes.band_pass(1, float(self.sample_rate), (float(self.bandpass_freq)-float(self.bandpass_width)/2), (float(self.bandpass_freq)+float(self.bandpass_width)/2), (float(self.bandpass_width)/10), window.WIN_HAMMING, float(self.beta)))
