...
# Title: Iq Playback Single Usrp X410
...
class iq_playback_single_usrp_x410(gr.top_block):

    def __init__(self, filepath="", ip_address="192.168.40.2", sample_rate="4", tx_channel="A:0", tx_frequency="2425.715", tx_gain="70"):
        gr.top_block.__init__(self, "Iq Playback Single Usrp X410", catch_exceptions=True)
...
def argument_parser():
    ...
    parser.add_argument(
        "--tx-channel", dest="tx_channel", type=str, default="A:0",
...
def main(top_block_cls=iq_playback_single_usrp_x410, options=None):
    if options is None:
        options = argument_parser().parse_args()
    tb = top_block_cls(filepath=options.filepath, ip_address=options.ip_address, sample_rate=options.sample_rate, tx_channel=options.tx_channel, tx_frequency=options.tx_frequency, tx_gain=options.tx_gain)
