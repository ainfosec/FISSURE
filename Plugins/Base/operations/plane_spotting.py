...
class OperationMain(Operation):
    def __init__(
        self,
        icao: str = "",
        description: str = "Aircraft detected",
        node_uid: str = "",
        ...
        self.icao = str(icao or "").strip().lower()
        self.description = description or "Aircraft detected"

    async def run(self) -> None:
        if not self.icao:
            raise ValueError("icao is required")

        executable = os.path.expanduser("~/Installed_by_FISSURE/dump1090/dump1090")
        process = await asyncio.create_subprocess_exec(executable, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.STDOUT)
        ...
        try:
            while not self._stop:
                ...
                if self.icao in text.lower():
                    await _emit_detection(self, "plane_spotting", self.description, {"icao": self.icao, "matched_line": text.strip()})
                    return
        finally:
