...
class OperationMain(Operation):
    def __init__(
        self,
        keyword: str = "",
        frequency: float = 102.5,
        description: str = "RDS keyword detected",
        node_uid: str = "",
        ...
        self.description = description or "RDS keyword detected"

    def _script_path(self):
        version = get_library_version() or "maint-3.10"
        return os.path.join(FLOW_GRAPH_BASE_DIR, version, "rds_keyword", "RDS_FM_USRPB2x0_stdout.py")
    ...
    async def run(self) -> None:
        ...
        process = await asyncio.create_subprocess_exec(sys.executable, "-u", script, "--freq=" + str(self.frequency * 1e6), cwd=os.path.dirname(script), stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.STDOUT,)
        ...
        try:
            while not self._stop:
                ...
                if self.keyword in text:
                    await _emit_detection(self, "rds_keyword", self.description, {"keyword": self.keyword, "frequency_mhz": self.frequency, "matched_line": text.strip()})
                    return
        finally:
